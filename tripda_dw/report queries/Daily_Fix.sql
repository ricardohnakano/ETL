SELECT
	tprinc.report_date AS report_reference,
	10000 * EXTRACT(YEAR FROM tprinc.report_date) + 100 * EXTRACT(MONTH FROM tprinc.report_date) + EXTRACT(DAY FROM tprinc.report_date) AS reference_number,
	EXTRACT(YEAR FROM tprinc.report_date) AS report_year,
	tprinc.country AS Country,
	sum(tb1.pax_transported) AS Pax_Transported_Sort,
...
FROM
	...
	CROSS JOIN
		...
		FROM
			users u
		WHERE
			date(u.created_at)=date(now())-1 OR
			date(u.created_at)=date(now())-8 OR
			date(u.created_at)=date(now())-15 OR
			date(u.created_at)=date(now())-22
		GROUP BY
...
FULL JOIN
	...
	GROUP BY
		...
		departure_country.name
	ORDER BY
		date(tl.departure_datetime) )tbs ON tprinc.report_date=tbs.trip_date AND tprinc.country=tbs.country
FULL JOIN
	(SELECT
		date(tl.departure_datetime) AS trip_date,
		departure_country.name AS country,
	...
	GROUP BY
		...
		departure_country.name
	ORDER BY
		date(tl.departure_datetime) )tb ON tprinc.report_date=tb.trip_date AND tb.country=tprinc.country
FULL JOIN
	(SELECT
		departure_country.name AS country,
		sum(tb.number_of_seats) AS pax_transported
	...
	WHERE
		...
		tb.rejected_at IS NULL AND
		tl.is_canceled='0'AND
		tb.canceled_at IS NULL AND
		date(tl.departure_datetime)=date(now())-1
	GROUP BY
		departure_country.name ) tb1 ON tb1.country=tprinc.country
FULL JOIN
	(SELECT
	count(*) AS new_users,
	(CASE WHEN u.locale='en_US'THEN'United States'
...
FULL JOIN
	...
	WHERE
		u.guid NOT IN('5788b25a-fbb6-4ee3-b2e0-c6d239dada08','93a33b54-fd34-4575-ad72-3028b7e368b3','30932f00-9862-4039-aea3-20d052e7dc53','73535830-4412-4304-bc76-091361452f2b','2737e141-525c-40ad-9031-d632e9fe6d07','f22d4ee9-5960-4019-ac20-d0ce52ccaeff','5669b34c-67d3-4470-b93a-a00756a973b8','ae338e24-be9e-4c67-86be-e90c3fbfc57c','8fcc9eb1-602f-4464-be56-8ad1028c2c04','177ccacd-a363-40e5-a23c-e2bcc550d224','34cb2d85-81b5-4fc6-a596-1adcd8f47a2a','5cb8ad99-c476-4ba0-a2b0-a6560b7372b2','6591fa5a-8fd2-4ae8-832d-c520fbe9862d','e81a32a3-e2cb-4b3f-9f04-cc4db1ad678a','e0acbd91-9229-4411-8020-0ae56d6d9552','2d933580-ed88-4a81-ab4f-5cd12a997ff7','47c5ab88-4aea-4ce7-8c2e-291db3ca17bb','f877ab95-d146-430d-a7d3-4ee188bc089e','f5d631b2-cdb0-4a6f-befb-d8001d2ad383','1e9cc6bb-cd3f-4d09-8b17-daa982a317ea') AND
		u.blocked IS FALSE
	GROUP BY
		date(tc.created_at),
...
WHERE
	date(tprinc.report_date)=date(now())-1 OR
	date(tprinc.report_date)=date(now())-8 OR
	date(tprinc.report_date)=date(now())-15 OR
	date(tprinc.report_date)=date(now())-22 
GROUP BY
	tprinc.report_date,
	tprinc.country
ORDER BY
	tprinc.country, 
	tprinc.report_date DESC
