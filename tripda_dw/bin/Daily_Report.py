...
def extract():
    ...
    daily_query = open(DATA_DIR + 'daily_query.sql','r').read()
...
def html (daily_report, index):
    ...
    arr = daily_report[['country','report_date','pax_transported_sort','trip_realized', \
                             'pax_transported','driver_rejection','driver_cancelation','pax_cancelation',\
                             'trip_offered','seats_km','new_trip_offered','new_bookings','new_users','new_drivers', \
                             'new_passangers','ask','average_distance','avarage_pax_per_trip','avg_pax/trip','Realize_Trip_Yield', \
                             'Avg_Realized_Km','Driver_Cancel_Rate','Pax_Cancel_Rate','sort']].to_dict()


    rowNumber = max(daily_report.index)
    rows = daily_report.index

    headerQuery = ['country','report_date','new_users','new_drivers','new_passangers','new_trip_offered','new_bookings', \
    ...
    index_default_format = ['new_users','new_drivers','new_passangers','new_trip_offered','ask','new_bookings','trip_realized','pax_transported']
